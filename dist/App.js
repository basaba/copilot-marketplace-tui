...
export default function App() {
    ...
    const [mpPlugins, setMpPlugins] = useState({});
    // Toast
    ...
    const summary = useMemo(() => computeSummary(plugins, marketplaces), [plugins, marketplaces]);
    // Navigation
    ...
    const [mpFocus, setMpFocus] = useState("tabbar");
    const [settingsCursor, setSettingsCursor] = useState(0);
    // Track which marketplaces have been fetched to avoid redundant loads
    const [mpFetched, setMpFetched] = useState(new Set());
    const [mpLoading, setMpLoading] = useState(new Set());
    // Track which marketplaces have had descriptions backfilled
    const [mpDescFetched, setMpDescFetched] = useState(new Set());
    ...
    useEffect(() => {
        if (screen !== "marketplace" || marketplaces.length === 0)
            return;
        const mp = marketplaces[mpTab];
        if (!mp || mpFetched.has(mp.name) || mpLoading.has(mp.name))
            return;
    ...
    const installPlugin = useCallback((p) => {
        if (p.installed || loading)
            return;
        ...
        const onSuccess = () => {
            ...
            setMpPlugins(prev => {
                ...
                updated[key] = (updated[key] || []).map(mp => mp.name === p.name ? { ...mp, installed: true } : mp);
                return updated;
            });
        ...
        copilot.installPluginAsync(spec).then(result => {
            ...
            if (result.success)
                ...
                showToast(`✗ Install failed: ${p.name}`);
        });
    }, [loading, showToast]);
    const uninstallPlugin = useCallback((p) => {
        if (loading)
            return;
        const spec = `${p.name}@${p.marketplace}`;
        const onSuccess = () => {
            ...
            setMpPlugins(prev => {
                ...
                for (const key of Object.keys(updated)) {
                    updated[key] = updated[key].map(mp => mp.name === p.name ? { ...mp, installed: false } : mp);
                }
                return updated;
            });
        ...
        copilot.uninstallPluginAsync(spec).then(result => {
            ...
            if (result.success)
                ...
                showToast(`✗ Uninstall failed: ${p.name}`);
        });
    }, [loading, showToast]);
    const enablePlugin = useCallback((p) => {
        if (loading)
            return;
        const spec = `${p.name}@${p.marketplace}`;
        const onSuccess = () => {
            setPlugins(prev => prev.map(pl => pl.name === p.name ? { ...pl, enabled: true } : pl));
        ...
        copilot.enablePluginAsync(spec).then(result => {
            ...
            if (result.success)
                ...
                showToast(`✗ Enable failed: ${p.name}`);
        });
    }, [loading, showToast]);
    const disablePlugin = useCallback((p) => {
        if (loading)
            return;
        const spec = `${p.name}@${p.marketplace}`;
        const onSuccess = () => {
            setPlugins(prev => prev.map(pl => pl.name === p.name ? { ...pl, enabled: false } : pl));
        ...
        copilot.disablePluginAsync(spec).then(result => {
            ...
            if (result.success)
                ...
                showToast(`✗ Disable failed: ${p.name}`);
        });
    }, [loading, showToast]);
    const updatePlugin = useCallback((p) => {
        if (!p.updateAvailable || loading)
            return;
        const spec = `${p.name}@${p.marketplace}`;
        const onSuccess = () => {
            setPlugins(prev => prev.map(pl => pl.name === p.name ? { ...pl, updateAvailable: false } : pl));
        ...
        copilot.updatePluginAsync(spec).then(result => {
            ...
            if (result.success)
                ...
                showToast(`✗ Update failed: ${p.name}`);
        });
    }, [loading, showToast]);
    // Get filtered lists for cursor bounds
    const filteredInstalled = useMemo(() => filterInstalled(plugins, instSearch), [plugins, instSearch]);
    const activeMarketplace = marketplaces[mpTab]?.name || "";
    const filteredMp = useMemo(() => filterMarketplace(mpPlugins[activeMarketplace] || [], mpSearch), [mpPlugins, activeMarketplace, mpSearch]);
    const screens = ["dashboard", "installed", "marketplace", "settings"];
    ...
    const renderScreen = () => {
        ...
        switch (screen) {
            case "dashboard":
                return (_jsx(Dashboard, { summary: summary }));
            case "installed":
                return (_jsx(InstalledView, { plugins: plugins, cursor: instCursor, searchQuery: instSearch, searchActive: instSearchActive, onSearchChange: (v) => {
                        setInstSearch(v);
                        setInstCursor(0);
                    }, termHeight: termHeight }));
            case "marketplace":
                return (_jsx(MarketplaceView, { marketplaces: marketplaces, plugins: mpPlugins, activeTab: mpTab, cursor: mpCursor, searchQuery: mpSearch, searchActive: mpSearchActive, onSearchChange: (v) => {
                        setMpSearch(v);
                        setMpCursor(0);
                    }, contentFocused: mpFocus === "content", termHeight: termHeight, loadingMarketplaces: mpLoading }));
            case "settings":
                return _jsx(SettingsView, { marketplaces: marketplaces, cursor: settingsCursor, addActive: addMpActive, addValue: addMpValue, onAddChange: setAddMpValue });
        }
    };
    if (!ready) {
        return (_jsx(Box, { flexDirection: "column", width: termWidth, height: termHeight, paddingX: 1, paddingY: 1, justifyContent: "center", alignItems: "center", children: _jsxs(Box, { children: [_jsx(Text, { color: "#58a6ff", children: _jsx(Spinner, { type: "dots" }) }), _jsx(Text, { color: "#58a6ff", children: " Loading plugins and marketplaces\u2026" })] }) }));
    }
    return (_jsxs(Box, { flexDirection: "column", width: termWidth, height: termHeight, paddingX: 1, paddingY: 1, children: [!showDetail && _jsx(TabBar, { active: screen, onSwitch: setScreen, focused: screen !== "marketplace" || mpFocus === "tabbar" }), loading && (_jsxs(Box, { marginBottom: 1, children: [_jsx(Text, { color: "#58a6ff", children: _jsx(Spinner, { type: "dots" }) }), _jsxs(Text, { color: "#58a6ff", children: [" ", loading] })] })), !loading && toast && (_jsx(Box, { marginBottom: 1, children: _jsx(Box, { paddingX: 1, children: _jsx(Text, { bold: true, color: toast.startsWith("✗") ? "#f85149" : "#3fb950", children: toast }) }) })), _jsx(Box, { flexDirection: "column", flexGrow: 1, children: renderScreen() })] }));
}
